...
module "vpc_tokyo" {
  ...
  # Enable Tokyo-specific configurations
  is_tokyo = true
  
  # Use only the available AZs in Tokyo
  availability_zones = ["ap-northeast-1a", "ap-northeast-1c"]
  
  # CIDR blocks for subnets
  private_cidr_blocks = {
    private1 = "10.100.10.0/24"  # Syslog subnet in ap-northeast-1a
    private2 = "10.100.11.0/24"  # Database subnet in ap-northeast-1c
  }
  
  # Public subnets will only be created in AZs not used for syslog or DB
  public_cidr_blocks = {
    public1 = "10.100.0.0/24"    # Will only be created if AZ is available
  }
...
module "vpc_california" {
  ...
  # California only has two AZs available
